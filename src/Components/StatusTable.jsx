        ...
        <tbody>
          {status.map((s, idx) => (
            <tr key={idx} className="border-t">
              ...
              <td className="px-2 py-2">{s.CreatedAt ? new Date(s.CreatedAt).toLocaleDateString() : '-'}</td>
              <td className="px-2 py-2">{s.Task || '-'}</td>
              <td className="px-2 py-2">{s.Assigned || '-'}</td>
              <td className="px-2 py-2">{s.Delivery_Date ? new Date(s.Delivery_Date).toLocaleDateString() : '-'}</td>
            </tr>
          ))}
        </tbody>
      </table>
